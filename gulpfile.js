...
var path = {
  ...
  src: {
    ...
    stylelight: 'src/assets/scss/style-light.scss',
...
gulp.task('csslight:dev', function () {
  return gulp.src(path.src.stylelight)
    .pipe(newer(path.dev.style))
    .pipe(plumber())
    .pipe(sass()
      .on('error', function (err) {
...
gulp.task('csslight:dist', function () {
  return gulp.src(path.src.stylelight)
    .pipe(newer(path.dist.style))
    .pipe(plumber())
    //.pipe(sourcemaps.init())
...
gulp.task('watch', function () {
    ...
    gulp.watch(path.watch.css, gulp.series('css:dist', 'csslight:dist'));
